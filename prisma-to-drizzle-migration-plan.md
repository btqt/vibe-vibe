...
**建议替换为：**
```markdown
| Drizzle Kit Studio | 可视化 Drizzle 数据库 |
```
...
- **类型**：`serial`（自增整数）、`text`（文本）、`boolean`（布尔）、`timestamp`（时间）
...
部署时也无需额外的构建步骤，直接使用默认的 `next build` 即可。
```
...
## Drizzle vs Prisma 关键差异

需要特别说明的点：

1. **无需 generate 步骤**：这是 Drizzle 的最大优势，需要在第7章和第11章明确说明
2. **Schema 格式**：从 `.prisma` DSL 改为 TypeScript 代码
3. **项目结构**：从 `prisma/` 文件夹改为 `src/db/` 目录
4. **初始化方式**：不需要 `init` 命令，直接安装依赖
5. **迁移命令**：从 `prisma migrate` 改为 `drizzle-kit push/migrate`
6. **构建命令**：从 `npx prisma generate && next build` 简化为 `next build`
